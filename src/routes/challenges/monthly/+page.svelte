		...
		<div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
			...
			<div class="text-center max-w-3xl mx-auto">
				...
				<h1 class="font-display font-bold text-5xl md:text-6xl mb-6 text-neutral-900 dark:text-neutral-50">
					Build Production <span class="bg-gradient-to-r from-purple-600 via-pink-600 to-rose-600 bg-clip-text text-transparent">Applications</span>
				</h1>
				
				<p class="text-xl text-neutral-600 dark:text-neutral-400 mb-8">
					{data.challenges.length} full-stack challenges. Spend 25-40 hours building production-ready applications.
				</p>
			...
			<div class="grid md:grid-cols-2 gap-8">
				{#each filteredChallenges as challenge, i (challenge.slug)}
					<div in:fly={{ y: 20, duration: 400, delay: i * 50 }}>
						<ChallengeCard {challenge} index={i} category="monthly" />
					</div>
				{/each}
			</div>
		{/if}
