  ...
  <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    {#if !isUnlocked && !isCompleted}
      <div class="mb-8 p-8 rounded-3xl bg-gradient-to-br from-orange-50 to-red-50 dark:from-orange-950/30 dark:to-red-950/30 border-2 border-orange-200 dark:border-orange-800 text-center">
        ...
        <div class="flex items-center justify-center gap-2 text-sm text-neutral-600 dark:text-neutral-400">
          ...
          {#if data.challenge.unlockDate}
            <span>Unlocks on {new Date(data.challenge.unlockDate).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</span>
          {:else}
        ...
        <div class="flex flex-wrap items-center gap-4 text-sm text-neutral-500 dark:text-neutral-500">
          ...
          {#if isCompleted && completedDate}
            <div class="flex items-center gap-2 text-green-600 dark:text-green-400 font-semibold">
              ...
              <span>Completed {new Date(completedDate).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</span>
            </div>
          {/if}
        </div>
      </div>
    ...
    {#if isUnlocked || isCompleted}
      ...
      {#if data.challenge.resources && data.challenge.resources.length > 0}
        <div class="mb-8 p-6 rounded-2xl bg-white dark:bg-neutral-900 border-2 border-neutral-200 dark:border-neutral-800 hover:border-green-300 dark:hover:border-green-700 transition-colors">
          <h3 class="font-bold text-lg mb-4 text-neutral-900 dark:text-neutral-50 flex items-center gap-2">
          ...
          <div class="space-y-3">
            {#each data.challenge.resources as resource}
              <a
                ...
                class="group flex items-center justify-between p-4 rounded-xl bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800 hover:bg-green-100 dark:hover:bg-green-900/40 transition-colors"
              >
                <span class="font-medium text-neutral-900 dark:text-neutral-50 group-hover:text-green-700 dark:group-hover:text-green-300 transition-colors">
                  {resource.name}
                </span>
                <svg class="w-5 h-5 text-green-600 dark:text-green-400 group-hover:scale-110 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor">
