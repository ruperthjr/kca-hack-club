      ...
      {#if data.challenge.recommendedFor && data.challenge.recommendedFor.length > 0}
        <div class="bg-white dark:bg-neutral-900 rounded-2xl border-2 border-purple-200 dark:border-purple-800 p-6 hover:shadow-xl transition-shadow">
          <div class="flex items-center gap-3 mb-4">
            ...
            <h3 class="font-bold text-lg text-neutral-900 dark:text-neutral-50">Recommended For</h3>
          </div>
          <div class="space-y-3">
            {#each data.challenge.recommendedFor as person}
              <div class="flex items-center gap-3 p-3 rounded-lg bg-gradient-to-r from-purple-50 to-fuchsia-50 dark:from-purple-950/30 dark:to-fuchsia-950/30 border border-purple-100 dark:border-purple-900">
                <div class="w-10 h-10 rounded-full bg-gradient-to-br from-purple-400 to-fuchsia-500 flex items-center justify-center text-white font-bold shadow-lg">
                  {person.charAt(0)}
                </div>
                <span class="text-neutral-900 dark:text-neutral-50 font-semibold">{person}</span>
              </div>
            {/each}
          </div>
        </div>
          ...
          {#each data.relatedChallenges as related}
            ...
            >
              <div class="text-sm font-semibold mb-2 {difficultyConfig[related.difficulty as keyof typeof difficultyConfig].text}">
                {difficultyConfig[related.difficulty as keyof typeof difficultyConfig].icon} {related.difficulty}
              </div>
